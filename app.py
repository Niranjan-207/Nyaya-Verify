"""
app.py — Nyaya-Verify  |  Legal Intelligence Clinical Dashboard
================================================================
UI/UX: Dark-themed dual-pane clinical dashboard.
  Left  (60%): Verified Legal Answer + Citation Footer
...
Pipeline:
  ...
  3. LegalSynthesizer → Ollama llama3.1:8b answer generation
...
# ─── Page Config ──────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Nyaya-Verify | Legal Intelligence Dashboard",
    page_icon="⚖️",
    layout="wide",
...
with st.sidebar:
    ...
    st.markdown(
        ...
        'margin-bottom:0.1rem;">⚖️ Nyaya-Verify</h2>'
        '<p style="color:#8b949e;font-size:0.78rem;margin-top:0;">'
        'Legal Intelligence Dashboard</p>',
        unsafe_allow_html=True,
    )
    ...
    # ── Active Collection ──────────────────────────────────────────────────────
    active_doc = st.session_state.get("active_doc", "nyaya_legal (53 statutes)")
    st.markdown(
    ...
    # ── PDF Ingestion ──────────────────────────────────────────────────────────
    st.markdown(
        '<span class="section-label">📥 Ingest New Statute</span>',
        unsafe_allow_html=True,
    )
    uploaded_pdf = st.file_uploader(
        "Drop a PDF statute here",
        type=["pdf"],
        label_visibility="collapsed",
    )
    year_str = st.text_input(
        "Statute year (optional)",
...
with st.form("query_form", clear_on_submit=False):
    query = st.text_area(
        "Legal Query",
        placeholder=(
            "e.g. What is the punishment for theft under BNS 2023 "
            "compared to IPC 1860?"
        ),
        height=110,
        label_visibility="collapsed",
    )
...
if submitted and not query.strip():
    st.warning("⚠️ Please enter a legal query before submitting.")
    st.stop()
...
if submitted and query.strip():
    ...
    if conflict.get("ConflictFound"):
        ...
        st.warning(
            f"⚠️ **LOGIC FLIP DETECTED** — Conflict between "
            f"`{r_meta.get('filename', 'Unknown')}` and "
            f"`{p_meta.get('filename', 'Unknown')}`. "
            f"Prioritising **{p_meta.get('statute_year', '?')}** mandate."
        )
    ...
    with left_col:
        st.markdown(
            '<span class="section-label">📋 Verified Legal Answer</span>',
            unsafe_allow_html=True,
        )
        ...
        for i, v in enumerate(verdicts, 1):
            ...
            with st.expander(
                ...
                st.markdown(
                    ...
                    f'📄&nbsp;{fname}&nbsp;·&nbsp;'
                    f'Page&nbsp;{pg}&nbsp;·&nbsp;'
                    f'Statute Year:&nbsp;{m.get("statute_year","?")}'
                    f'</div>',
                    unsafe_allow_html=True,
                )
